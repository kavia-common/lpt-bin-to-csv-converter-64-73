...
class ParserConfigRequest(BaseModel):
    ...
    skip_header: bool = Field(
        default=False,
        description="Skip header parsing"
    )
...
class ConversionResponse(BaseModel):
    ...
    records_converted: int = Field(
        ...,
        description="Number of records converted to CSV",
        examples=[1000, 50000]
    )
    
    input_file: str = Field(
        ...,
        description="Path to input file that was converted"
    )
...
class HealthResponse(BaseModel):
    ...
    message: str = Field(
        default="LPT.bin to CSV Converter API is running",
