...
# PUBLIC_INTERFACE
def stream_records(
    file_handle, config: ParserConfig, max_records: Optional[int] = None
):
    """
    ...
    while True:
        ...
        if len(record_bytes) < record_size:
            logger.warning(
                f"Incomplete record at end of file (byte {file_handle.tell()})"
            )
            break
...
def convert_lpt_bin_to_csv(
    input_path: str, output_path: str, config: Optional[ParserConfig] = None
) -> Dict[str, any]:
    """
    ...
    try:
        with open(input_path, "rb") as bin_file:
            ...
            if not config.skip_header:
                try:
                    ...
                    logger.warning(
                        f"Header parsing failed: {str(e)}. Attempting to parse as raw records."
                    )
        ...
        logger.info(
            f"Conversion complete: {records_converted} records written to {output_path}"
        )
